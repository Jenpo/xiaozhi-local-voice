...
几十块到一两百块的 AI 玩具机器人（xiao-zhi / xiaozhi-esp32 生态）出厂默认是**全云端方案**：设备只负责录音和播放，语音识别、对话、语音合成全部在厂商服务器上完成（白牌产品里相当一部分直接接豆包这类国内云 API）。开箱即用，但延迟、隐私和可用性都攥在别人手里。

社区方案因此是**自建服务端**：把 ASR / LLM / TTS 都放到自己局域网里。这一步解决了掌控权，却也带来一个常见后果——服务端被塞进一台低功耗 NAS，对话走局域网里共享的大模型服务，语音合成仍调用云端 TTS。每一环单独看都能跑通，拼在一起就是：孩子说完一句话，要等 **15—20 秒**才听到回应。对一个五六岁的孩子来说，这个等待时间已经足够让他转身走开。
